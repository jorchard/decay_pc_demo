...
class Connection(object):
    ...
    def Load(self, fp):
        W_decay = np.load(fp).item()
        M_decay = np.load(fp).item()
        self.SetDecay(W_decay=W_decay, M_decay=M_decay)

        learn = np.load(fp).item()
        self.learn = learn
